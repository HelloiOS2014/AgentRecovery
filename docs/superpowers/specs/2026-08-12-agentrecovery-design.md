...
- 2026-08-12 v5：picker 项目归属（真实使用反馈）。此前列表 cwd 恒为「?」且按全局时间序排列，用户在当前项目内 /recover 时看不到本项目的会话（可能被挤出前 20）。修复：`list_sessions` 对返回子集读取首条 session_meta 填充 cwd（只读少量文件首行，非全库扫描）；列表按「cwd 是否匹配当前目录（realpath 比较）」置顶（stable，组内仍按时间序），当前项目条目标 `*`；`show` 对跨项目会话在渲染前输出来源警告。序号解析与列表共用同一排序，保持一致。
